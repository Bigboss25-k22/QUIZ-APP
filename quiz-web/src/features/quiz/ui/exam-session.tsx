...
function ExamSessionContent({ data, userId }: { data: QuizDetail; userId: number }) {
  ...
  return <main className="min-h-screen bg-paper"><header className="sticky top-0 z-20 border-b border-line bg-white"><div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-3 px-5"><ButtonLink intent="quiet" size="sm" href="/tests"><X size={17} aria-hidden="true"/>Thoát phiên</ButtonLink><p className="hidden truncate font-display text-sm font-bold sm:block">{data.testDTO.title}</p><ExamTimer seconds={seconds}/></div></header><div className="mx-auto max-w-7xl p-5"><Progress label={`Đã trả lời ${answeredCount}/${data.questions.length} câu`} value={(answeredCount / data.questions.length) * 100}/><div className="mt-5 grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]"><QuestionPanel active={active} total={data.questions.length} question={question} answer={answers[String(question.id)]} flagged={flagged.includes(question.id)} isLast={active === data.questions.length - 1} pending={submit.isPending} onAnswer={(value) => selectAnswer(question.id, value)} onFlag={() => toggleFlag(question.id)} onPrevious={() => goToQuestion(active - 1)} onNext={() => goToQuestion(active + 1)} onSubmit={finish}/><QuestionNavigator active={active} questions={data.questions} answers={answers} flagged={flagged} onSelect={goToQuestion}/></div>{submit.isError && <StatusMessage className="mt-5" tone="error">Không thể nộp bài. Câu trả lời vẫn được giữ; hãy thử lại.</StatusMessage>}</div></main>;
}
