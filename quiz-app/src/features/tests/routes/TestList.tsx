...
function TestListPage() {
  ...
  const { data: response, error: queryError, isLoading: loading } = useTests({
    page,
    size: 10,
  ...
  const errorMessage = queryError?.response?.data?.message || queryError?.message;
  ...
  return (
    <div className="max-w-6xl mx-auto pt-32 pb-10 px-4">
        ...
        {errorMessage && (
          <div className="text-center text-red-500 font-semibold py-8">{errorMessage}</div>
        )}
