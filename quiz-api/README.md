...
- Gradle Wrapper (đã có trong source, không cần cài Gradle riêng)
- PostgreSQL 12+
...
Tạo file `.env` trong thư mục `quiz-api`:

```env
# Application
SPRING_APPLICATION_NAME=quiz-api
SERVER_PORT=8082
...
```properties
spring.application.name=${SPRING_APPLICATION_NAME:quiz-api}
...
Test chạy với profile `test` và H2 in-memory, không cần PostgreSQL local.

### Run Specific Test Class

```bash
./gradlew test --tests com.quiz.api.AuthServiceTest
```
...
Báo cáo HTML được tạo tại `build/reports/jacoco/test/html/index.html`.
```
...
│   ├── auth/
│   │   ├── api/                 # HTTP contract and controller
│   │   ├── application/         # authentication use cases and ports
│   │   ├── domain/              # refresh-token model
│   │   └── infrastructure/      # JWT and JPA adapters
│   ├── user/
│   │   ├── api/
│   │   ├── application/
│   │   ├── domain/              # User and UserRole
│   │   └── infrastructure/      # persistence and initialization adapters
│   ├── quiz/
│   │   ├── api/
│   │   ├── application/
│   │   ├── domain/              # Quiz, Question and QuizResult
│   │   └── infrastructure/      # JPA adapters
│   ├── shared/                  # pagination, exceptions and shared contracts
│   ├── config/                  # Spring configuration
│   ├── security/                # JWT and Spring Security adapters
│   └── web/                     # global HTTP adapters
│   └── QuizApiApplication.java
└── src/main/resources/
...
java -jar build/libs/quiz-api-0.0.1-SNAPSHOT.jar
```
...
docker run -p 8082:8082 --env-file .env quiz-api
```
